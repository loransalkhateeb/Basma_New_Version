...
exports.getCouponById = async (req, res) => {
  try {
    ...
    if (!coupoun) {
      return res.status(404).json(ErrorResponse("Coupoun not found", [`No Coupoun with ID: ${id}`]));
    }

    res.status(200).json([coupoun]);
  } catch (error) {
    console.error("Error fetching Coupoun by ID:", error.message);
    res.status(500).json(ErrorResponse("Error fetching Coupoun by ID", [error.message]));
  }
};
