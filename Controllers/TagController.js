...
exports.getTagById = async (req, res) => {
  try {
    ...
    if (data) {
      ...
      if (!tag) {
        return res.status(404).json( ErrorResponse("Tag not found", ["No tag found with the given id"]));
      }
    ...
    console.error(error);
    res.status(500).json( ErrorResponse("Failed to fetch tag", ["An error occurred while fetching the tag"]));
  }
};
...
exports.deleteTag = async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json(ErrorResponse("Failed to delete tag", ["An error occurred while deleting the tag"]));
  }
};
